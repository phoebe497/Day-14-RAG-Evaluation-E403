...
class LLMJudge:
    ...
    def score_response(
        ...
        import json
        prompt = (
            f"You are an AI Judge evaluating a response. Here is the rubric:\n"
            f"{json.dumps(rubric, indent=2)}\n\n"
            f"Question: {question}\n"
            f"Response to evaluate: {answer}\n\n"
            f"Output a JSON object mapping each rubric criterion to a score between 0.0 and 1.0."
        )
        try:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if scores:
                item_averages.append(sum(scores) / len(scores))
            else:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            if pair.retrieved_contexts:
                eval_result.context_precision = evaluator.evaluate_context_precision(
                    pair.retrieved_contexts, pair.expected_answer
                )
                eval_result.context_recall = evaluator.evaluate_context_recall(
                    pair.retrieved_contexts, pair.expected_answer
                )
...
class FailureAnalyzer:
    ...
    def categorize_failures(
        ...
        categories = {}
        for f in failures:
            ft = f.failure_type if f.failure_type is not None else "unknown"
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_score = min(f, r, c)
        min_count = sum(1 for v in (f, r, c) if v == min_score)
    ...
    def generate_improvement_log(self, failures: list[EvalResult], suggestions: list[str]) -> str:
        ...
        for idx, failure in enumerate(failures):
            ...
            if suggestions:
                ...
                fix = "Review and optimize system pipeline components"
                
            rows.append(f"| {f_id} | {f_type} | {root_cause} | {fix} | Open |")
            
        header_str = "| " + " | ".join(headers) + " |"
        align_str = "| " + " | ".join(align) + " |"
        
        return "\n".join([header_str, align_str] + rows)
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0) > 0 or any(f.failure_type and f.failure_type.lower() == "hallucination" for f in failures):
            suggestions.append("Implement hallucination checker or self-correction loop to filter unsupported claims")
            
        if categories.get("incomplete", 0) > 0 or any(f.failure_type and f.failure_type.lower() == "incomplete" for f in failures):
            suggestions.append("Increase chunk size or retrieve more documents in RAG pipeline to reduce context fragmentation")
            suggestions.append("Add few-shot examples showing complete answers to improve generation completeness")
            
        if categories.get("irrelevant", 0) > 0 or any(f.failure_type and f.failure_type.lower() == "irrelevant" for f in failures):
            suggestions.append("Improve prompt clarity and specify constraints to ensure the agent answers the exact question")
            
        if categories.get("off_topic", 0) > 0 or any(f.failure_type and f.failure_type.lower() == "off_topic" for f in failures):
            suggestions.append("Refine intent classification or routing logic to prevent off-topic responses")
            
        fallback_suggestions = [
            "Optimize retrieval model using hybrid search (BM25 + Dense vector search) to fetch better source documents",
            "Integrate reranking (such as Cohere or BGE Cross-Encoder) to prioritize relevant chunks",
            "Add guardrails to catch out-of-scope queries and handle adversarial inputs gracefully"
        ]
